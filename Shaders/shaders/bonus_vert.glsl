...
void main(){
  ...
  normalInterp = vec3(normalMat * vec4(normal,1.0));//vertex normal in camera space
  vertPos = vec3(modelview * vec4(position,1.0)); //vertex position in camera space
  vec3 eye = vec3(modelview * vec4(eyePos,1.0)); //eye position in camera space
  viewVec = eye-vertPos; //eye to vertex
}
